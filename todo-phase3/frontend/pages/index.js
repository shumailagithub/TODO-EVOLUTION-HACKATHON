...
export default function Home() {
  ...
  // Helper function to ensure task objects have required properties
  const normalizeTask = (task) => ({
    id: task.id || '',
    serial_number: task.serial_number || 0,
    title: task.title || '',
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const normalizedTask = normalizeTask(data.task);
        setTasks(tasks.map(task =>
          task.id === editingTask ? { ...task, ...normalizedTask } : task
        ));
        setEditingTask(null);
        setEditTitle('');
        setEditDescription('');
                  ...
                  >
                    {editingTask === task.id ? (
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        >
                          {task.completed === true ? <Check size={20} /> : <Circle size={20} />}
                        </motion.button>
        ...
        <div className="fixed bottom-8 right-8 z-50">
          <Link href="/chat">
            <motion.button
              ...
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              className="w-16 h-16 rounded-full bg-gradient-to-r from-purple-500 to-blue-500 text-white shadow-2xl flex items-center justify-center hover:shadow-3xl transition-all duration-300"
              title="AI Chatbot"
            >
