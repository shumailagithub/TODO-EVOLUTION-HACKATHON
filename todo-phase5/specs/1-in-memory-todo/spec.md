...
- **NFR-001**: The system MUST respond to commands within 100 milliseconds for up to 1000 tasks
  - **Measurement Method**: Create 1000 tasks using the `add` command, then measure response time for `list`, `complete`, `update`, and `delete` commands using Python's `time.perf_counter()`. Each command must complete in < 100ms.
  - **Acceptance Criteria**: All commands (add, list, complete, update, delete) respond in < 100ms when 1000 tasks exist in memory.

- **NFR-002**: The system MUST be simple enough for a non-technical user to learn within 5 minutes
  - **Measurement Method**: Provide a new user with only the `help` command output. Time how long it takes them to successfully: (1) add a task, (2) view all tasks, (3) mark a task as complete. Success = all three operations completed correctly.
  - **Acceptance Criteria**: 3 out of 3 test users can complete all three basic operations within 5 minutes using only the help output as guidance.
